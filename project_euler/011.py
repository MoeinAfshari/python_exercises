...
# text grid converts to number grid
def grid(grid):
    split_nums = [x.split() for x in (grid.strip().split("\n"))]
    grid = []
    for i in split_nums:
        grid += [list(map(int, i))]
    return grid

# production of grid elements
def grid_product(ox, oy, dx, dy, n):
    resault = 1
    for i in range(n):
        resault *= GRID[oy + i * dy][ox + i * dx]
    return resault

# this function find largest product in the GRID
def multiple(num):
    ans = -1
    width = len(GRID[0])
    height = len(GRID)
